  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="flex w-full items-center justify-between gap-2 rounded-lg border border-white/15 bg-ink/40 px-2 py-2 text-left text-sm text-cream transition hover:border-volt/60"
      >
        <span className="truncate">{selectedLabel}</span>
        <CaretDownIcon size={16} className="shrink-0 text-cream-dim" />
      </button>

      {open && (
        <div className="absolute z-[1000] mt-1 w-full overflow-hidden rounded-lg border border-white/15 bg-ink-soft shadow-xl shadow-black/40">
          <div className="flex items-center gap-2 border-b border-white/10 px-2">
            <MagnifyingGlassIcon size={16} className="shrink-0 text-cream-dim" />
            <input
              ref={inputRef}
              value={query}
            ...
            />
            <span className="shrink-0 text-xs text-cream-dim">{matches.length}</span>
          </div>

          <ul className="max-h-72 overflow-y-auto py-1">
            {matches.length === 0 && (
              <li className="px-3 py-2 text-sm text-cream-dim">No matches</li>
            )}
            {matches.map((pt, i) => {
              ...
              return (
                <li key={`${ptKey(pt)}-${i}`}>
                  {showGroup && (
                    <div className="px-3 pb-0.5 pt-2 text-[11px] font-medium uppercase tracking-wide text-cream-dim">
                      {pt.group}
                    </div>
                  )}
                  <button
                    ...
                    className={`flex w-full items-center justify-between gap-2 px-3 py-1.5 text-left text-sm text-cream ${
                      i === activeIdx ? "bg-white/10" : ""
                    }`}
                  >
                    <span className="flex items-center gap-2 truncate">
                      {pt.label}
                      <span className="truncate text-xs text-cream-dim">{ptKey(pt)}</span>
                    </span>
                    {isSel && <CheckIcon size={14} className="shrink-0 text-volt" />}
                  </button>
                </li>
              );
            })}
